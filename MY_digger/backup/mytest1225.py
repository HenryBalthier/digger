...
class MainStrategy(Strategy):
    ...
    def on_init(self, ctx):
        ...
        ctx.ttin = Turtle(ctx.close, tt['IN'], 'ttin', ('r', 'g'))
        ctx.ttout = Turtle(ctx.close, tt['OUT'], 'ttout', ('y', 'b'))
...
class RUN(object):
    ...
    def results(self):
        ...
        for pf in self.rst:
            ...
            # K-line
            MY_digger.myplot.plot_strategy(self.pf[pf[0]].data(0), self.pf[pf[0]].technicals(0), self.pf[pf[0]].deals(0),
                                           curve.equity, figname)

            # Equity-curve
            MY_digger.myplot.plot_curves([curve.equity], colors=['r'], names=[self.pf[pf[0]].name(0) + '-equity'])
            MY_digger.myplot.plot_curves([curve.cash], colors=['g'], names=[self.pf[pf[0]].name(0) + '-cash'])
            MY_digger.myplot.plot_curves([curve.returns], colors=['b'], names=[self.pf[pf[0]].name(0) + '-returns'])
            MY_digger.myplot.plot_curves([curve.equity, curve.cash, curve.returns, curve.commission],
                                         colors=['r', 'g', 'b', 'y'],
