...
class TT(TechnicalBase):
    """ 布林带指标。 """
    @tech_init
    def __init__(self, data, n, name='TT',
            styles=('y', 'b', 'g'), lw=1):
        super(TT, self).__init__(name)
    ...
    def plot(self, widget):
        """ 绘图，参数可由UI调整。 """
        self.widget = widget
        self.plot_line(self.values['min'], self.styles[0], lw=self.lw)
        self.plot_line(self.values['max'], self.styles[1], lw=self.lw)
        #self.plot_line(self.values['lower'], self.styles[2], lw=self.lw)


class Turtle_base(TechnicalBase):
    """ Turtle Strategy"""
    @tech_init
    def __init__(self, data, n, name='Turtle', style=('y', 'g'), lw=1):
        super(Turtle_base, self).__init__(name)
        self._args = [ndarray(data), n]

    def _vector_algo(self, data, n):
        min_n, max_n = talib.MINMAX(data, n)
        self.values = {'max': max_n, 'min': min_n}

    def plot(self, widget):
        self.widget = widget
        self.plot_line(self.values['max'], self.style[0], lw=self.lw)
        self.plot_line(self.values['min'], self.style[1], lw=self.lw)
...
__all__ = ['MA', 'BOLL', 'Volume', 'Line', 'LineWithX', 'Turtle_base', 'TT']
